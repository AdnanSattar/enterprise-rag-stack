...
from .retrieval import get_retriever, get_vector_store
from .schemas import (
    ConfidenceLevel,
    DeleteRequest,
    DeleteResponse,
...
from .verification import VerificationStatus, verify_answer
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info(f"Starting RAG Service v{__version__}")
...
async def query_endpoint(body: QueryRequest, request: Request):
    ...
    try:
        ...
        # 2. Rerank
        if body.rerank:
            # Use lightweight reranker by default for speed
        ...
        # Cache result
        cache.set(cache_key, result.model_dump(), ttl=settings.CACHE_TTL)
...
async def search_endpoint(body: SearchRequest):
    ...
    """
    from .chunking import num_tokens
    ...
    try:
        ...
        if result is None:
            return IngestResponse(
                success=False,
                errors=["Failed to process file or duplicate detected"]
            )
    ...
    try:
        ...
        for result in results:
            try:
                ...
                errors.append(f"Error processing {result.metadata.get('source_path')}: {e}")
...
async def delete_documents(body: DeleteRequest):
    ...
    try:
        ...
        # Find all chunk IDs for these documents
        ...
        for doc_id in body.doc_ids:
            try:
                ...
                # In production, you'd query for all chunks with this doc_id
